...
class _MammalsMedicalState extends ConsumerState<MammalsMedical> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ref.watch(vaccinationListProvider(widget.animal.id!)).when(
        ...
        ref.watch(breedingEventListProvider(widget.animal.id!)).when(
            error: (error, trace) => Text('Error: $error'),
            loading: () => const CircularProgressIndicator(),
            data: (events) {
              final lastEvent = events.lastOrNull;
        ...
        ref.watch(vaccinationListProvider(widget.animal.id!)).when(
            ...
            data: (vaccinations) {
              return Column(
                children: vaccinations.map((vaccination) => StyledDismissible(
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        if (vaccination.files != null &&
                            vaccination.files!.isNotEmpty)
                          IconButton(
                            onPressed: () {
                              Navigator.push(
                                  context,
                    ...
                    subtitle: vaccination.firstDoseDate != null ||
                        vaccination.secondDoseDate != null
                        ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          vaccination.firstDoseDate != null
        ...
        ref
            ...
            data: (checkups) {
              return Column(
                children: checkups.map((checkup) => StyledDismissible(
                  confirmDismiss: _confirmDeletion,
                  onDismissed: (direction) {
                    ref
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        if (checkup.files != null &&
                            checkup.files!.isNotEmpty)
                          IconButton(
                            onPressed: () =>
                                _showFile(checkup.files!, 0),
                            icon: const Icon(
                              Icons.file_copy_outlined,
                              color: AppColors.primary40,
                            ),
                    ...
                    subtitle: checkup.firstCheckUp != null ||
        ...
        ref.watch(surgeryListProvider(widget.animal.id!)).when(
            error: (error, trace) => Text('Error: $error'),
            loading: () => const CircularProgressIndicator(),
            data: (surgeries) {
              return Column(
                children: surgeries.map((surgery) => StyledDismissible(
                  confirmDismiss: _confirmDeletion,
                  onDismissed: (direction) {
                    ref
                  ...
                  child: ListTile(
                    ...
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        if (surgery.files != null &&
                            surgery.files!.isNotEmpty)
                          IconButton(
                            onPressed: () =>
                                _showFile(surgery.files!, 0),
                            icon: const Icon(
                              Icons.file_copy_outlined,
                              color: AppColors.primary40,
                            ),
                    ...
                    subtitle: surgery.firstSurgery != null ||
                        surgery.secondSurgery != null
                        ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          DateFormat('yyyy-MM-dd').format(
                              surgery.firstSurgery ??
                                  DateTime.now()),
                          style: AppFonts.body2(
                              color: AppColors.grayscale70),
                        ),
